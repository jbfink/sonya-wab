  ...
  var clusterLayer = declare('ClusterLayer', [GraphicsLayer, Evented], {
    ...
    _flashFeature: function (feature) {
      if (feature.geometry) {
        ...
        var size = this._getSymbolSize(feature);
        var symbol = new SimpleMarkerSymbol(SimpleMarkerSymbol.STYLE_CIRCLE, size,
    ...
    _getSymbolSize: function (feature) {
      var sourceLayer = feature.getSourceLayer ? feature.getSourceLayer() : undefined;
      var size;
      if (feature.symbol && feature.symbol.size) {
        size = feature.symbol.size;
      } else if (sourceLayer && sourceLayer.renderer &&  sourceLayer.renderer.symbol && sourceLayer.renderer.symbol.size) {
        size = sourceLayer.renderer.symbol.size;
      } else {
        size = 16;
      }
      return size;
